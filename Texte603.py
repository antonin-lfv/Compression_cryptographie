...
from Chiffrement.ChiffrementParDecalage import *
from Chiffrement.compression_par_repetition import *
...
class Texte603(object):
    ...
    def exTexte603(num=1):
        if num == 0:
            res = "aababcbbdaababcbbdbaaacabdebf"
        elif num == 1:
            ...
            with open('fichiers/GuerreEtPaix UTF8.txt', 'r') as f:
                res = f.read()
        else:
    ...
    def afficheCompressionParDico(self):
        ...
        print(
            f"Liste des {len(lmots)} mots d'un dico de {len(ldico)}:")  # on cherche tous les w dans self et renvoie une liste de ceux-ci
...
if __name__ == "__main__":
    ...
    print("---- Test du chiffrement par décalage ----")
    texte = Texte603.exTexte603()
    print("Texte : ", texte)
    text_to_binaire = texte.toBinaire603()
    code = Chiffrement_par_decalage(1).binCode(text_to_binaire)
    print("Texte encodé : ", code)
    decode = Chiffrement_par_decalage(1).binDecode(code)
    print("Texte décodé : ", decode)
    final = Texte603(decode)
    print("Final : ", final)

    print("\n\n---- Test du chiffrement par décalage ----")
    texte = Texte603.exTexte603()
    print("Texte : ", texte)
    text_to_binaire = texte.toBinaire603()
    code = Compression_par_repetition().binCode(text_to_binaire)
    print("Texte encodé : ", code)
    ...
    print("Entropie du texte de départ : ", round(text_to_binaire.entropie(), 3))
    print("Entropie du texte encodé : ", round(code.entropie(), 3))
